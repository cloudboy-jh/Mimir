...
func Recall(ctx context.Context, opts RecallOptions) (RecallResult, error) {
	dir, query, budget := opts.Dir, opts.Query, opts.Budget
	if budget <= 0 {
		budget = 4000
	}
	...
	if err != nil {
		return RecallResult{}, fmt.Errorf("missing .mimir/index.json; run mimir index --full")
	}
	matches := rank(idx, query)
	return RecallResult{Query: query, Budget: budget, Stale: info.Stale, Matches: fit(matches, budget)}, nil
}
...
func Format(res RecallResult) string {
