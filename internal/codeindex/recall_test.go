...
func TestRecallEndToEnd(t *testing.T) {
	...
	if _, err := Build(context.Background(), BuildOptions{Dir: root, Full: true}); err != nil {
		t.Fatal(err)
	}
	res, err := Recall(context.Background(), RecallOptions{Dir: root, Query: "resolveSession", Budget: 4000})
	if err != nil {
		t.Fatal(err)
	}
