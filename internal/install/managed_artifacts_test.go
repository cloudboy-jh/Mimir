...
func TestInstallFromDifferentNonTemporaryExecutablePreservesBinaryOwnership(t *testing.T) {
	paths := isolatedInstallation(t, false)
	ownedPath := filepath.Join(t.TempDir(), "owned-mimir")
	ownedData := []byte("owned binary")
	invoker := filepath.Join(t.TempDir(), "other-mimir")
	if err := os.WriteFile(ownedPath, ownedData, 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(invoker, []byte("other verified Mimir"), 0o755); err != nil {
		t.Fatal(err)
	}
	receipt := newInstallReceipt()
	receipt.Source, receipt.Method = "release", "bootstrap-copy"
	receipt.CLI = installReceiptCLI{Path: ownedPath, Version: "1.0.0", Hash: hashBytes(ownedData)}
	if err := writeJSONAtomic(paths.Receipt, receipt); err != nil {
		t.Fatal(err)
	}
	old := isMimirExecutable
	isMimirExecutable = func(path string) bool { return sameFilePath(path, invoker) }
	oldTemporary := executableIsTemporary
	executableIsTemporary = func(string) bool { return false }
	t.Cleanup(func() { isMimirExecutable = old; executableIsTemporary = oldTemporary })

	report, err := Install("", func() (string, error) { return invoker, nil })
	if err != nil {
		t.Fatal(err)
	}
	if report.Binary.Path != invoker {
		t.Fatalf("invoking binary = %q", report.Binary.Path)
	}
	after, err := loadInstallReceipt()
	if err != nil {
		t.Fatal(err)
	}
	if after.CLI != receipt.CLI || after.Source != receipt.Source || after.Method != receipt.Method {
		t.Fatalf("binary ownership transferred: %#v", after)
	}
}

func TestInstallAdoptsVerifiedNonTemporaryExecutableWithoutOwner(t *testing.T) {
	isolate := isolatedInstallation(t, false)
	_ = isolate
	binary := filepath.Join(t.TempDir(), "mimir")
	data := []byte("verified Mimir")
	if err := os.WriteFile(binary, data, 0o755); err != nil {
		t.Fatal(err)
	}
	old := isMimirExecutable
	isMimirExecutable = func(path string) bool { return sameFilePath(path, binary) }
	oldTemporary := executableIsTemporary
	executableIsTemporary = func(string) bool { return false }
	t.Cleanup(func() { isMimirExecutable = old; executableIsTemporary = oldTemporary })
	if _, err := Install("", func() (string, error) { return binary, nil }); err != nil {
		t.Fatal(err)
	}
	receipt, err := loadInstallReceipt()
	if err != nil {
	...
	if !sameFilePath(receipt.CLI.Path, binary) || receipt.CLI.Hash != hashBytes(data) {
		t.Fatalf("verified executable was not adopted: %#v", receipt.CLI)
	}
}
...
func TestRefreshManagedInstallationDoesNotTransferCLIToDifferentExecutable(t *testing.T) {
	...
	SetBuildInfo(BuildInfo{Version: "2.0.0", Commit: "new-commit", Date: "new-date"})
	t.Cleanup(func() {
		executablePath = oldExecutablePath
		SetBuildInfo(BuildInfo{Version: oldVersion, Commit: oldCommit, Date: oldDate})
	})
...
func TestRefreshManagedInstallationDoesNotAdoptReplacedBinaryInPlace(t *testing.T) {
	...
	SetBuildInfo(BuildInfo{Version: "2.0.0", Commit: "new", Date: "new-date"})
	t.Cleanup(func() {
		executablePath = oldExecutablePath
		SetBuildInfo(BuildInfo{Version: oldVersion, Commit: oldCommit, Date: oldDate})
	})
...
func TestUninstallRemovesUnchangedPreservesModifiedAndUpdatesReceiptAndLog(t *testing.T) {
	...
	if !bytes.Contains(logData, []byte(`"operation":"uninstall"`)) || !bytes.Contains(logData, []byte(`"binary":`)) || !bytes.Contains(logData, []byte(`"summary":`)) {
