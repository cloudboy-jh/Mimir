...
2. Check for `mimir`. If absent, run `go run github.com/cloudboy-jh/mimir/cmd/mimir@latest install`. The installed binary embeds the Worker, plugins, and skills; do not locate a Worker in the Go module cache.
...
9. For OpenCode, allow the installer to manage only its exact receipt-owned plugin and skill files. For OpenCode provider/MCP settings and harnesses without a bundled integration, use the harness's supported configuration flow and the connection manifest. Supply authentication using `credential_file`, `credential_command`, or secure secret input. Never directly rewrite general OpenCode JSON/JSONC, providers, credentials, commands, or MCP entries.
