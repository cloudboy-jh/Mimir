...
import { extractGeneratedTitle, finalizedExchangeTitleStatements } from "./session-titles";
...
export async function capture(env: Bindings, input: CaptureInput): Promise<void> {
  ...
  const titleCandidate = requestKind === "title" ? extractGeneratedTitle(redactedResponse) : null;
  const latency = Date.now() - input.started;
  if (!await prepareAcceptedExchange(env.DB, id, session.id, prepared, latency, titleCandidate)) return;
  ...
  try {
    await finalizeAcceptedExchange(env.DB, id, session.id, activityAt, new Date().toISOString(), input.harness, input.model, prepared.usage.prompt_tokens, prepared.usage.completion_tokens, r2Bytes, true, titleCandidate);
  } catch (error) {
...
async function prepareAcceptedExchange(db: D1Database, exchangeId: string, sessionId: string, prepared: PreparedCapture, latency: number, titleCandidate: string | null) {
  try {
    await db.batch([
      db.prepare("UPDATE exchanges SET response_excerpt = ?, usage_json = ?, latency_ms = ?, provider = ?, finish_reason = ?, input_tokens = ?, output_tokens = ?, title_candidate = ? WHERE id = ? AND capture_status = 'accepted'")
        .bind(prepared.responseExcerpt, JSON.stringify(prepared.usage), latency, prepared.provider, prepared.finishReason, prepared.usage.prompt_tokens, prepared.usage.completion_tokens, titleCandidate, exchangeId),
...
export async function finalizeAcceptedExchange(db: D1Database, exchangeId: string, sessionId: string, activityAt: string, savedAt: string, harness: string | null, model: string, inputTokens: number, outputTokens: number, r2Bytes: number | null, reactivate: boolean, generatedTitle: string | null = null) {
  await db.batch([
    ...
    ...finalizedExchangeTitleStatements(db, sessionId, generatedTitle, activityAt),
  ]);
}
