...
import { readSaveConfig, decideCapture } from "../config/config-store";
import type { AppEnv } from "../env";
import { capture } from "../exchanges/capture-pipeline";
import type { RequestKind } from "../exchanges/exchange-types";
import { readBoundedText } from "../exchanges/response-codec";
import { SESSION_ID } from "../sessions/events";
...
export async function proxy(
  ...
  if (declaredSession && !SESSION_ID.test(declaredSession))
    return c.json({ error: "invalid x-mimir-session" }, 400);
  const requestKindHeader = c.req.header("x-mimir-request-kind");
  if (requestKindHeader && !REQUEST_KINDS.has(requestKindHeader as RequestKind))
    return c.json({ error: "invalid x-mimir-request-kind" }, 400);
  ...
  const upstream = await fetch(
    `https://openrouter.ai/api/v1${endpoint === "chat" ? "/chat/completions" : "/messages"}`,
  ...
  if (decision.capture === "skipped" || !upstream.body)
  ...
  c.executionCtx.waitUntil(
    capture(c.env, {
      ...
      console.error(
        JSON.stringify({
          message: "exchange capture failed",
