...
export function shouldSave(
  ...
  return (
    config.enabled &&
    !config.excludeRepos.some((value) => matches(value, repo ?? "")) &&
    !config.excludeModels.some((value) => matches(value, model))
  );
}
...
export function decideCapture(
  ...
  if (config.excludeRepos.some((value) => matches(value, repo ?? "")))
    return { capture: "skipped", reason: "excluded_repository" };
  if (config.excludeModels.some((value) => matches(value, model)))
    return { capture: "skipped", reason: "excluded_model" };
  if (!hasResponseBody)
    return { capture: "skipped", reason: "missing_response_body" };
...
export function validateConfigValues(values: Record<string, unknown>) {
  for (const [key, value] of Object.entries(values)) {
    ...
    if (key === "save.enabled" && typeof value !== "boolean")
      return `${key} must be boolean`;
    if (
      ["save.exclude_repos", "save.exclude_models", "redact.patterns"].includes(
        key,
      )
    ) {
      if (
        ...
        return `${key} must be an array of strings up to 256 characters`;
    }
    if (
      key === "session.gap_minutes" &&
      (typeof value !== "number" ||
        !Number.isInteger(value) ||
        value < 1 ||
        value > 1440)
    )
      return `${key} must be an integer from 1 to 1440`;
  }
  return "";
}
