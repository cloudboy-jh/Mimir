...
import { extractGeneratedTitle, normalizeSessionTitle } from "./titles";

describe("session titles", () => {
  it("normalizes human and model title forms", () => {
    expect(normalizeSessionTitle("  ## Title:  Fix   capture races  ")).toBe(
      "Fix capture races",
    );
    expect(normalizeSessionTitle('"Ship session titles"')).toBe(
      "Ship session titles",
    );
  ...
  it("extracts OpenAI, Anthropic, and reconstructed streaming titles", () => {
    ...
    ).toBe("Anthropic title");
    expect(
      extractGeneratedTitle({ stream: true, content: "Streamed title" }),
    ).toBe("Streamed title");
