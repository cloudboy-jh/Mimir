...
export function extractGeneratedTitle(response: unknown): string | null {
  ...
  const direct = normalizeSessionTitle(
    record.content ?? record.output_text ?? record.output,
  );
  ...
  for (const choice of choices) {
    ...
    const candidate = normalizeSessionTitle(
      message.content ?? choiceRecord.text,
    );
  ...
  const blocks = content
    .map((block) => objectRecord(block).text)
...
export function titleUpdateStatement(
  db: D1Database,
  sessionId: string,
  title: string,
  ...
  return db
    .prepare(
      `UPDATE sessions SET title = ?, title_source = ?, title_updated_at = ? WHERE id = ? AND (
    title IS NULL OR
...
export function finalizedExchangeTitleStatements(
  db: D1Database,
  sessionId: string,
...
export function reconcileSessionTitleStatement(
  db: D1Database,
  sessionId: string,
  updatedAt: string,
) {
