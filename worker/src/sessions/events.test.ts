...
describe("parseSessionEvent", () => {
  it("accepts turn, heartbeat, and end events", () => {
    ...
    ).toMatchObject({ parent_session_id: "session-root" });
  });

  it("rejects malformed envelopes", () => {
    expect(parseSessionEvent(null)).toEqual({
      error: "event must be an object",
    });
    expect(parseSessionEvent([1])).toEqual({
      error: "event must be an object",
    });
    expect(parseSessionEvent({ ...base, version: 2, kind: "turn" })).toEqual({
      error: "unsupported event version",
    });
    expect(parseSessionEvent({ ...base, kind: "note" })).toEqual({
      error: "invalid event kind",
    });
    expect(
      parseSessionEvent({ ...base, kind: "turn", session_id: "bad id!" }),
    ).toEqual({ error: "invalid session_id" });
    expect(
      parseSessionEvent({
        ...base,
        kind: "heartbeat",
    ...
    ).toEqual({ error: "invalid parent_session_id" });
    expect(
      parseSessionEvent({ ...base, kind: "turn", ts: "not-a-date" }),
    ).toEqual({ error: "invalid ts" });
  });
  ...
  it("validates turn fields and caps the excerpt", () => {
    ...
    ).toEqual({ error: "invalid turn usage" });
    expect(
      parseSessionEvent({
        ...base,
        kind: "turn",
    ...
    const event = parseSessionEvent({
      ...
      turn: {
        exchange_id: "ex-1",
        provider: "OpenAI",
        usage: { input_tokens: 5.7, output_tokens: 3 },
        latency_ms: 12.9,
    ...
    expect((event as { turn: { excerpt: string } }).turn.excerpt).toHaveLength(
      500,
    );
  });
  ...
  it("normalizes timestamps and defaults the end reason", () => {
    ...
    expect(
      parseSessionEvent({ ...base, kind: "end", reason: "user closed" }),
    ).toMatchObject({ reason: "user closed" });
  });
  ...
  it("accepts optional harness-native titles without changing the v1 envelope", () => {
    ...
    ).toMatchObject({ version: 1, title: "Fix session titles" });
    expect(
      parseSessionEvent({ ...base, kind: "heartbeat" }),
    ).not.toHaveProperty("title");
    expect(
      parseSessionEvent({ ...base, kind: "heartbeat", title: " " }),
    ).toEqual({ error: "invalid title" });
    expect(
      parseSessionEvent({ ...base, kind: "heartbeat", title: "x".repeat(201) }),
    ).toEqual({ error: "invalid title" });
  });
});
