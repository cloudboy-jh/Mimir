...
describe("captureReceipt", () => {
  ...
  it("uses compact copy for every user-visible capture state", () => {
    expect(receipt("saved", 1, 0, 0)).toEqual({
      label: "Saved to Mimir",
      detail: "1 exchange in this session",
      action_label: "View session",
    });
    expect(receipt("pending", 2, 0, 1)).toEqual({
      label: "Saving to Mimir...",
      detail: "3 exchanges",
      action_label: "View session",
    });
    expect(receipt("partial", 2, 1, 0)).toEqual({
      label: "Partially saved",
      detail: "2 of 3 exchanges",
      action_label: "View details",
    });
    expect(receipt("failed", 0, 2, 0)).toEqual({
      label: "Mimir couldn't save this session",
      detail: "2 exchanges",
      action_label: "View details",
    });
    expect(receipt("empty", 0, 0, 0)).toEqual({
      label: "Not captured",
      detail: "No exchanges in this session",
      action_label: "View session",
    });
    expect(receipt("pending", 1, 1, 1)).toEqual({
      label: "Partially saved",
      detail: "1 saved · 1 failed · 1 pending",
      action_label: "View details",
    });
  });
  ...
  it("only advertises a deep link when dashboard Access is available", () => {
    ...
    expect(
      sessionStatusResponse(
        "https://mimir.example/sessions/id/status",
        "session/id",
        summary,
        { outcome: "unresolved" },
        true,
