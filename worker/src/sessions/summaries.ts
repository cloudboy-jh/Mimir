...
export function buildSessionSummary(input: SummaryInput): string | null {
  ...
  else if (input.outcome === "discarded")
    parts.push("The recorded work was discarded.");
  else if (input.outcome === "abandoned")
    parts.push("The session ended without a retained result.");
  ...
  if (input.file_count)
    facts.push(
      `${input.file_count} changed ${input.file_count === 1 ? "file" : "files"}`,
    );
  if (input.error_count)
    facts.push(
      `${input.error_count} recurring ${input.error_count === 1 ? "error" : "errors"}`,
    );
...
export async function ensureSessionSummary(
  ...
  const summaryAt =
    typeof session.summary_updated_at === "string"
      ? Date.parse(session.summary_updated_at)
      : 0;
  const evidenceAt = Math.max(
    ...[
      session.outcome_updated_at,
  ...
  if (
    ...
    (session.summary_status === "ready" && summaryAt >= evidenceAt)
  )
    return session;
  ...
  await db
    .prepare(
      "UPDATE sessions SET summary_text = ?, summary_status = ?, summary_source = 'generated', summary_updated_at = ? WHERE id = ?",
    )
    .bind(summary, summary ? "ready" : "unavailable", now, session.id)
    .run();
  return {
    ...session,
    summary_text: summary,
    summary_status: summary ? "ready" : "unavailable",
