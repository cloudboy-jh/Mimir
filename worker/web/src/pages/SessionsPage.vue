    ...
    <div class="overflow-hidden rounded-[7px] border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
      ...
      <template v-else>
        <div v-for="block in blocks" :key="block.root.id" class="border-b border-zinc-200 last:border-b-0 dark:border-zinc-800">
          <div class="group relative grid gap-3 px-4 py-4 transition-colors hover:bg-stone-50 lg:grid-cols-[minmax(0,1fr)_150px_130px_150px_90px_28px] lg:items-center dark:hover:bg-stone-900/40">
            ...
            <div class="pointer-events-none relative z-10 text-left font-mono text-xs text-zinc-700 lg:text-right dark:text-zinc-300"><span class="mr-1 text-zinc-500 lg:hidden">Tokens</span>{{ compactNumber(block.root.tokens_in + block.root.tokens_out) }}</div><ArrowRight class="pointer-events-none relative z-10 hidden size-4 text-zinc-400 transition-transform group-hover:translate-x-0.5 lg:block" aria-hidden="true" />
          </div>
