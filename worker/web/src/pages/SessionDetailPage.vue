...
<template>
  <section v-if="session">
    ...
    <div class="grid gap-8 pt-8 xl:grid-cols-[minmax(0,1fr)_320px]">
      <div><h2 class="mb-4 text-sm font-semibold text-zinc-900 dark:text-zinc-100">Request timeline</h2><div class="border-t border-zinc-200 dark:border-zinc-800"><RouterLink v-for="exchange in detail?.exchanges" :key="exchange.id" :to="`/requests/${exchange.id}`" class="grid gap-3 border-b border-zinc-200 py-4 hover:bg-stone-50 focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-teal-600 sm:grid-cols-[120px_minmax(0,1fr)_100px] sm:px-3 dark:border-zinc-800 dark:hover:bg-zinc-900"><time class="font-mono text-xs text-zinc-500">{{ shortDate(exchange.ts) }}</time><div><div class="flex flex-wrap items-center gap-3"><IdentityBadge :label="exchange.provider || 'Unknown provider'" /><IdentityBadge :label="exchange.model" /></div><p class="mt-2 font-mono text-xs text-zinc-500">{{ exchange.id }}<template v-if="exchange.finish_reason"> · {{ exchange.finish_reason }}</template></p></div><div class="font-mono text-xs text-zinc-500 sm:text-right">{{ exchange.input_tokens.toLocaleString() }} in</div></RouterLink><p v-if="!detail?.exchanges.length" class="py-10 text-sm text-zinc-500">No request evidence was captured for this session.</p></div></div>
