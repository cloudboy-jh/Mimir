...
- Keep terminal presentation code under `internal/ui/` with dependency direction `mimircli -> receipts / lineoutput -> appframe -> bentotui`. Mimir has no terminal application, raw-input loop, session browser, or agent TUI; the dashboard is the visual browser and agents query the machine-readable CLI. All command output is static or append-only. See `internal/ui/README.md`.
