...
- Keep terminal UI code under `internal/ui/` with dependency direction `mimircli -> surfaces -> appframe -> bentotui`. Stateful terminal commands use the shared, top-left-anchored app frame (80x20 preferred, 48x12 minimum). `mimir install` and `mimir update` are deliberately plain line-oriented commands, and `mimir demo` is a browser-first line-oriented command; none are TUI surfaces. Never center or indent frames, add nested boxes, or duplicate frame and key logic in command files. See `internal/ui/README.md`.
...
- Shared PNG assets: `assets/images/`. Worker materialization must preserve assets imported by the dashboard. Regenerate the README hero and dashboard wordmark with `go run ./scripts/generate-readme-image.go`.
