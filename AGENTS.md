...
- Keep terminal UI code under `internal/ui/` with dependency direction `mimircli -> surfaces -> appframe -> bentotui`. Stateful human commands use the shared, top-left-anchored app frame (80x20 preferred, 48x12 minimum). `mimir install` and `mimir update` are deliberately plain line-oriented commands, not TUI surfaces. Never center or indent frames, add nested boxes, or duplicate frame and key logic in command files. See `internal/ui/README.md`.
